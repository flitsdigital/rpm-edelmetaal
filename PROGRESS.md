...
| 21 | **n8n-workflow `7ok70BWJ7aHF1FNU`** — webhook → normaliseren → honeypot/volledigheidscheck → data table → mail → 200 | Gebouwd via de n8n MCP op verzoek. De aanvraag wordt éérst opgeslagen (data table `WkEqFiX9I8noEaB2`) en pas daarna gemaild, met `onError: continueRegularOutput` — een maildstoring mag geen lead kosten. | 31-07-2026 |
| 25 | **Verzenden via SMTP i.p.v. Gmail** | Op verzoek. Send Email-node op de Flits-credential `info@flitsdigital.nl`. Reply-to staat op de aanvrager, dus als RPM op de melding antwoordt komt dat bij de klant terecht en niet bij Flits. | 03-08-2026 |
